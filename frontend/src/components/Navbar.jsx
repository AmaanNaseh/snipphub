...
const Navbar = ({ theme, setTheme }) => {
  ...
  return (
    ...
    >
      ...
      {isTechnologies && (
        <div
          className={`hidden z-50 overflow-auto lg:flex fixed top-[80px] left-0 w-full max-h-[75vh] p-20 items-center justify-start gap-40 flex-wrap border-y-[1px] border-y-gray-300/25 
      ...
      {isSideNav ? (
        <div
          className={`lg:hidden z-50 overflow-auto fixed top-[80px] left-0 w-full max-h-[75vh] flex flex-col gap-4 p-8 border-y-[1px] border-y-gray-300/25 ${
            theme === "dark" ? "bg-[#0d1117]" : "bg-white"
          }`}
        >
          {Commands.map((item, index) => {
